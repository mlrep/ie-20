...
#colors = [green2, blue2, red3]
colors = [red3, blue2, green2]
markers = ['', '', '', '']
line_width = [0.6, 0.7, 0.8, 0.6]  #[1.0, 0.7, 0.7, 0.6]
...
def create_plot(input_path, sel_series, size, g, ag, p, w, omit, lb_path, labels_map, output_name, legend,
                ylabel, title, xticks=None, yticks=None, ylim=None, smooth=1):
    init_pgf()
    ...
    for alg in sel_series:
        ...
        vals = list(map(lambda v: v, vals))  # todo: 1-1.0 for error
        x = list(map(lambda x: (x * g) + init, range(0, len(vals))))

        og = int(omit/g)
        omit_val = x[og]

        y_smooth = gaussian_filter1d(vals[og::ag[i]], sigma=smooth)
        ax.plot(x[og::ag[i]], y_smooth, linewidth=line_width[i], color=colors[i], marker=markers[i], markersize=0.1,
                label=alg)
        i = i + 1
    ...
    if xticks is None:
        xticks = []
        if size == 500000:
            xticks = range(100000, 500000, 150000)
        elif size == 600000:
            xticks = range(100000, 600000, 200000)
        elif size == 1000000:
            xticks = range(300000, 1000000, 300000)
        elif size == 1200000:
            xticks = range(200000, 1100000, 400000)

    xticks_labels = []
    for tic in xticks:
        if 1000000 > tic >= 1000:
            xticks_labels.append(str(int(tic / 1000)) + 'k')
        else:
            xticks_labels.append(str(int(tic / 1000000)) + 'm')

    plt.xticks(xticks, xticks_labels)
    ax.grid(color='gray', linestyle='dashed', linewidth=0.6, alpha=0.2)
    plt.grid(True)

    save_fig('{0}'.format(output_name))
